...
api: 'POST /teams/{team_id}/chargers/{charger_id}/charge-limit'
...
<Tabs>
  <Tab title="Example">
    <CodeGroup>
      ...
      ```
      ```json 504 (Hub not reachable)
      {
        "message": "Cannot connect to the Hub, please try again later. Check the latest status on https://status.volttime.com."
      }
      ```
    </CodeGroup>
  </Tab>
