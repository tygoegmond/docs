...
description: 'Sets a limit on the amount of energy that can be drawn from a connector'
---
...
<Tabs>
  <Tab title="Example">
    <CodeGroup>
      ...
      ```json 504 (Timeout)
      {
        "message": "The charging device is not responding promptly. Alternatively, it's possible that a connection to the Hub cannot be established. Please retry later and monitor the latest status at https://status.volttime.com."
      }
      ```
    </CodeGroup>
  </Tab>
